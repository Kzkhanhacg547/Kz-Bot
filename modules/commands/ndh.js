...
module.exports.languages = {
    "vi": {
        ...
        "addedNewNDH": '𝗦𝘂𝗰𝗰𝗲𝘀𝘀 𝗔𝗱𝗱𝗲𝗱 %1 𝗡𝗗𝗛:\n\n=>> %2',
...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        case "add": {
            if (event.senderID != 100081129610697) return api.sendMessage(`Quyền lồn biên giới?`, event.threadID, event.messageID)
            if (permssion != 3) return api.sendMessage(getText("notHavePermssion", "add"), threadID, messageID);
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mention.length != 100081129610697 && isNaN(content[0])) {
                var listAdd = [];
        ...
        case "delete": {
            ...
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mentions.length != 100081129610697 && isNaN(content[0])) {
                const mention = Object.keys(mentions);
                var listAdd = [];
