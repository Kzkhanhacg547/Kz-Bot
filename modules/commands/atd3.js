    ...
    try {
     ...
     // const head = app => '';
      ...
      /* TỰ ĐỘNG TẢI ẢNH HOẶC VIDEO YOUTUBE */ 
                ...
                const res = await axios.get(`https://TPK2-API.last-namename.repl.co/youtube/downloader?url=${str}`);
                send({
      ...
      /*AUTODOWN CAPCUT VIIDEO */
                ...
                var res = (await axios.get(`https://mitai-project.ductaimaidinh2006.repl.co/capcut?url=${str}`))
...
👉 𝗕𝗮̣𝗻 𝗺𝘂𝗼̂́𝗻 𝗲𝗱𝗶𝘁 𝘃𝗶𝗱𝗲𝗼 𝘁𝗵𝗶̀ 𝗮̂́𝗻 𝘃𝗼̂ 𝗹𝗶𝗻𝗸 𝗰𝗮𝗽𝗰𝘂𝘁 𝗼̛̉ 𝘁𝗿𝗲̂𝗻 đ𝗲̂̉ 𝗲𝗱𝗶𝘁 𝗻𝗵𝗮́`, attachment: fs.createReadStream(path)},o.event.threadID,() => fs.unlinkSync(path),o.event.messageID)*/
      ...
      /* TỰ ĐỘNG TẢI ẢNH HOẶC NHẠC SOUNDCLOUD */ 
       ...
       var res = (await axios.get(`https://nguyenmanh.name.vn/api/scDL?url=${str}&apikey=p2tHfdSb`))
      ...
      /* TỰ ĐỘNG TẢI NHẠC ZINGMP3 */ 
          ...
          send({body: `${head('ZINGMP3')}\n`,attachment: await streamURL(`https://nguyenmanh.name.vn/api/zMp3DL?url=${str}&apikey=p2tHfdSb`, 'mp3')})
        }
      ...
      /* TỰ ĐỘNG TẢI ẢNH HOẶC VIDEO INSTAGRAM */ 
                ...
                const res = await axios.get(`https://apipremium-thanhali.thanhali.repl.co/twitter/download?url=${str}`);
                const {
                    videos = [{}],
                    images
                } = res.data;
