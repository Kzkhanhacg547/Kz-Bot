module.exports.config = {
    ...
    version: "1.0.0",
    hasPermssion: 2,
    credits: "D-Jukie - fix lại by LVBang",
...
module.exports.run = async function ({ api, event, args }) {
  const permission = ["100081129610697",""];
        ...
        axios.get(url[0]).then(i => {
            ...
            fs.writeFile(
                ...
                function (err) {
                    ...
                    api.sendMessage(`Đã áp dụng code vào ${args[0]}.js, sử dụng cmd load để sử dụng!`, threadID, messageID);
                }
            );
        })
    }
    ...
    if (url[0].indexOf('buildtool') !== -1 || url[0].indexOf('tinyurl.com') !== -1) {
        ...
        request(options, function (error, response, body) {
            ...
            load('.language-js').each((index, el) => {
                ...
                fs.writeFile(`./modules/commands/${args[0]}.js`, code, "utf-8",
                    function (err) {
                        if (err) return api.sendMessage(`Đã xảy ra lỗi khi áp dụng code mới cho "${args[0]}.js".`, threadID, messageID);
                        return api.sendMessage(`Đã thêm code này vào "${args[0]}.js", sử dụng event load để sử dụng!`, threadID, messageID);
                    }
                );
            });
        });
    ...
    if (url[0].indexOf('drive.google') !== -1) {
      ...
      const path = resolve(`./modules/commands/${args[0]}.js`);
      try {
