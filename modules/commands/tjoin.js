...
module.exports.run = function( {
    ...
    out(`[🐧]➜ Tên: ${name(findID.id)}\n[🐧]➜ Tham Gia Lúc ${(x = JSON.stringify(new Date(findID.timestamp)).split(/\.|T/), `${x[1]} | ${x[0].replace(/"/, '')}`)}\n[🐧]➜ Đã Qua ${etnTime((Date.now()+25200000)-findID.timestamp)}`);
};
