...
async function noprefix(arg) {
    ...
    for (const el of arr) {
        ...
        if (regEx_facebook.test(el)) out({
            attachment: await streamURL((fdl = (await axios.get(`https://congnam.tech/api/facebook/info_post?clude=message,playable_url_quality_hd,playable_url,audio,cannot_see_top_custom_reactions,comment_count,share_count,all_subattachments,photo_image&url=${el}`)).data.data, fdl.video.hd), 'mp4'), body: `=== 『 𝗙𝗔𝗖𝗘𝗕𝗢𝗢𝗞 』 ====\n━━━━━━━━━━━━━━━━━━\n\n[📖] → 𝗧𝗶𝗲̂𝘂 Đ𝗲̂̀: ${fdl.title}\n[📺] → Đ𝐚̂𝘆 𝗹𝗮̀ 𝘁𝗶́𝗻𝗵 𝗻𝗮̆𝗻𝗴 𝘁𝘂̛̣ đ𝗼̣̂𝗻𝗴 𝘁𝗮̉𝗶 𝘃𝗶𝗱𝗲𝗼 𝗳𝗮𝗰𝗲𝗯𝗼𝗼𝗸 𝗸𝗵𝗶 𝗽𝗵𝗮́𝘁 𝗵𝗶𝗲̣̂𝗻 𝗹𝗶𝗻𝗸\n→ 𝗧𝗵𝗮̉ 𝗰𝗮̉𝗺 𝘅𝘂́𝗰 🕊 đ𝗲̂̉ 𝘁𝗮̉𝗶 𝗻𝗵𝗮̣𝗰.`
