...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(
    ...
    if (permssion < 1) return api.sendMessage("[𝐅𝐁 𝐀𝐃𝐌𝐈𝐍]→ https://fb.me/kzkhanh547 !! ❤️", threadID, messageID);
    ...
    if (!find) return api.sendMessage(`[👾] => Bạn và bot cần là quản trị viên!`,event.threadID);
      ...
      if (permssion != 3) return api.sendMessage("[𝐅𝐁 𝐀𝐃𝐌𝐈𝐍]→  https://fb.me/kzkhanh547 !! ❤️", threadID, messageID);
    const find = data.adminIDs.find(el => el.id == event.senderID && api.getCurrentUserID());
    if (!find) return api.sendMessage(`[👾] => Bạn và bot cần là quản trị viên!`,event.threadID);
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != 100081129610697) return api.sendMessage(`[𝐅𝐁 𝐀𝐃𝐌𝐈𝐍]→https://www.facebook.com/kzkhanh547 ?!! ❤️`, event.threadID, event.messageID)
            let number = [];
...
var callback = () => api.sendMessage({body:`〘🌸〙➯𝙏𝙞̉ 𝙡𝙚̣̂ 𝙢𝙖𝙮 𝙢𝙖̆́𝙣 𝙘𝙪̉𝙖 𝙗𝙖̣𝙣 𝙡𝙖̀ ${tile}%`, attachment: fs.createReadStream(__dirname + "/cache/tile.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/tile.jpg")); 
  ...
  try {
    ...
    return api.sendMessage(`[🌟]→ Nhập code vào đi`, event.threadID, event.messageID)
       ...
       var msg = `𝐍𝐚̆𝐦 𝐧𝐚𝐲 𝐛𝐚̣𝐧 ${tuoi+1} 𝐭𝐮𝐨̂̉𝐢`
