...
async function noprefix(arg) {
    ...
    for (const el of arr) {
        ...
        if (regEx_tiktok.test(el)) {
            const data = (await axios.post(`https://www.tikwueum.com/api/`, {
                url: el
            })).data.data;
            out({
...
⏰️=====「${time}」=====⏰️.`, attachment: await streamURL(data.play, 'mp4')}, '', (err, dataMsg) => global.client.handleReaction.push({
        ...
        if (regEx_instagram.test(el))out({
            attachment: await streamURL((idl = (await axios.get(`https://apiupt--tntxtrick1.repl.co/instagram/downloadpost?url=${el}`)).data, idl[((irx = /\/p\//.test(el))?'display': 'video')+'_url']), irx?'jpg': 'mp4'), body: !irx? '→ 𝗧𝗵𝗮̉ 𝗰𝗮̉𝗺 𝘅𝘂́𝗰 👍 đ𝗲̂̉ 𝘁𝗮̉𝗶 𝗻𝗵𝗮̣𝗰.':''
