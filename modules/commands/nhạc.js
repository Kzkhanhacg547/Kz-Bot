module.exports.config = {
    ...
    hasPremssion: 0,
    credits: "KimiZK",
    description: "Random nhạc có sẵn",
...
module.exports.run = async ({ api, event }) => {
 const axios = require('axios');
  ...
  const fs = require("fs");
   //axios.get('https://kz-api.kzbott.repl.co/sound').then(res => {
 ...
 // let callback = function () {
  ...
  var callback = () =>
          api.sendMessage({
             body: `====== [ 𝗠𝗣𝟯 ] ======\n————————————\n➜ 𝗡𝗵𝗮̣𝗰 𝗰𝘂̉𝗮 𝗯𝗮̣𝗻 𝗱𝗮̂𝘆 :𝟯\n➜ 𝗛𝗶𝗲̣̂𝗻 𝘁𝗮̣𝗶 𝗱𝗮𝗻𝗴 𝗰𝗼́: ${link.length} 𝗯𝗮̀𝗶 𝗻𝗵𝗮̣𝗰 𝘁𝗿𝗲̂𝗻 𝗵𝗲̣̂ 𝘁𝗵𝗼̂́𝗻𝗴!\n➜ 𝗖𝗵𝘂́𝗰 𝗯𝗮̣𝗻 𝗺𝗼̣̂𝘁 𝗻𝗴𝗮̀𝘆 𝘁𝗼̂́𝘁 𝗹𝗮̀𝗻𝗵`, attachment: fs.createReadStream(__dirname + "/cache/5.mp3")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/5.mp3"));           
           ...
           // attachment: fs.createReadStream(__dirname + `/cache/mp3.${ext}`)
        //  }, event.threadID, () => fs.unlinkSync(__dirname + `/cache/mp3.${ext}`), event.messageID);
     //   };
      //  request(res.data.data).pipe(fs.createWriteStream(__dirname + `/cache/mp3.${ext}`)).on("close", callback);
   //   })
// }
